...
BASE_PREDICTION_FACTOR := 0.0040 ; Base prediction factor
PREDICTION_VARIATION := 0.0150 ; Variation range for prediction factor
...
global currentPredictionFactor := BASE_PREDICTION_FACTOR
...
AimbotLoop:
    ...
    if (!ErrorLevel && foundX >= left && foundX <= right && foundY >= top && foundY <= bottom) {
        ...
        predictedX := aimX + (velX * currentPredictionFactor)
        predictedY := aimY + (velY * currentPredictionFactor)

        ; Update the last found positions
        lastFoundX := foundX
        lastFoundY := foundY

        ; Calculate relative movement, factoring in DPI and sensitivity
        ...
        ; Randomly adjust the prediction factor slightly to avoid patterns but increases accuracy
        currentPredictionFactor := BASE_PREDICTION_FACTOR + (Random(-PREDICTION_VARIATION, PREDICTION_VARIATION))
    }
return
